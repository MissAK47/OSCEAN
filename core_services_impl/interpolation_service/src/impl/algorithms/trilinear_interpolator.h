...
/**
 * @brief 三线性插值算法实现
 * @details 支持SIMD加速的完整三线性插值算法
 */
class TrilinearInterpolator : public IInterpolationAlgorithm {
public:
    /**
     * @brief 构造函数
     * @param simdManager SIMD管理器（可选）
     */
    explicit TrilinearInterpolator(
...
private:
    ...
    /**
     * @brief 2D情况下的SIMD优化（当只有一个波段时）
     * @param sourceGrid 源网格数据
     * @param targetPoints 目标点集合
     * @return 插值结果
     */
    std::vector<std::optional<double>> interpolateAtPointsSIMD2D(
        const GridData& sourceGrid,
        const std::vector<TargetPoint>& targetPoints) const;
};
